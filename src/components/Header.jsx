...
function Header({setSearchQuery}) {
	...
	return (
		<div className="header">
			<Link className="link headerTitle" to="/index.html">
				<h2 className="headerLogo">Film Bulucu</h2>
			</Link>

			<div className="headerSearchContainer">
				<input className="headerSearchInput" type="text" placeholder="Ara..." name="search" results="5" id="search" onChange={(e) => setSearchQuery(e.target.value)}/>
			...
			<div className="headerNavContainer">
				<Link to={!user ? "/login" : ""} className="link">
					<div onClick={handleAuthenticaton} className="headerOption">
						<span className="headerOptionLineOne">{user ? user.displayName : ""}</span>
						<span className="headerOptionLineTwo">{user ? "Çıkış Yap" : "Giriş Yap"}</span>
					</div>
				</Link>
			</div>
