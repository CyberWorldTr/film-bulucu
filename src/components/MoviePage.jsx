...
function MoviePage({ id }) {
	...
	useEffect(() => {
		document.title = ` ${movie.title} | Film Bulucu`;
	}, [movie]);
		...
		<div className="moviePagecontainer">
			...
			<div className="foreground">
				...
				{Object.keys(movie).length !== 0 && movie.constructor === Object ? (
					<div className="movieContainer">
						<a href={`https://www.themoviedb.org/movie/${movie.id}`} target="_blank" rel="noreferrer" style={{ textDecoration: "none" }}>
							<div className="movie-Page">
								<span className="span" title={`Rating Ortalaması: ${avgRating}⭐`}>
						...
						<div className="movieContent">
							...
							<h4>
								{movie.release_date ? movie.release_date : ""} {movie.runtime ? `• ${movie.runtime}dk` : ""}<br/><br/>{movieRatings.length ? <> <i className="stars fas fa-star"/> {avgRating} • {movieRatings.length} oylama</> : "Henüz Oylama Yok!"}
							</h4>
							{movie.overview ? <h3>Film Özeti:</h3> : ""}
							<p className="overview">{movie.overview}</p>
							{movie.genres ? movie.genres.length ? <h3>Tür:</h3> : "" : ""}
							<p>
								{movie.genres &&
							...
							<div className="userRatings">
								<h5>Bu filmi izlediniz mi? Oylayın! </h5>
								{user ? (
									<h6>Puanınızı gönderin:</h6>
								) : (
									<h6>
					...
					"Malesef Aradığınız Filmi Bulamadık 😞"
				)}

				<div className="castOuter">
					{cast ? cast.length ? <h3 className="castTitle">Oyuncular:</h3> : "" : ""}
				...
				<div className="recommendedOuter">
					{recommended ? recommended.length ? <h3 className="recommendedTitle">Son Oyladıklarım:</h3> : "" : ""}
								...
								? recommended.slice(0, 8).map((movie, i) => (
										<Link to={`/movie/${movie.id}`} key={i} style={{ textDecoration: "none" }}>
											<div alt={movie.title} title={movie.title} className="recommendedMovie">
												...
												<div className="movieinfo">
													<h3>{movie.title}</h3>
													<h3 className="character">{(simScores[movie.id] * 100).toFixed(2)}% Eşleşme</h3>
												</div>
											</div>
										</Link>
								  ))
